...
def HD_encoder(train_features, val_features, test_features, args):
    ...
    train_HVs = torch.zeros(train_features.size(0), args.HV_dim)
    for sample_idx in range(train_features.size(0)):
        fea_HVs = emb_lvl(train_features[sample_idx, :, :])
        spatial_HVs = hd.hash_table(IM_HVs, fea_HVs)
        spatial_HVs = hd.hard_quantize(spatial_HVs)
        encoded_HV = hd.bind_sequence(spatial_HVs)

        train_HVs[sample_idx, :] = encoded_HV

    val_HVs = torch.zeros(val_features.size(0), args.HV_dim)
    for sample_idx in range(val_features.size(0)):
        ...
        val_HVs[sample_idx, :] = encoded_HV

    test_HVs = torch.zeros(test_features.size(0), args.HV_dim)
    for sample_idx in range(test_features.size(0)):
        ...
        test_HVs[sample_idx, :] = encoded_HV
