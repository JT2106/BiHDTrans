...
class BinarizationSign(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input, scaling_factor=None):
        ctx.save_for_backward(input, scaling_factor)
    ...
    def backward(ctx, grad_output):
        # 获取保存的输入数据
        input, scaling_factor = ctx.saved_tensors
        # 初始化梯度
        ...
        if scaling_factor is not None:
            grad_scaling_factor = grad_output.sum() * input.detach()
        else:
            grad_scaling_factor = None

        return grad_input, grad_scaling_factor


class BinarizationSignLayer(torch.nn.Module):
    def __init__(self, use_pws=False):
        super().__init__()
        self.use_pws = use_pws
        if use_pws:
    ...
    def forward(self, input):
        return BinarizationSign.apply(input, self.scaling_factor)


class SoftmaxAwareBinarization(autograd.Function):
    @staticmethod
    def jvp(ctx: Any, *grad_inputs: Any) -> Any:
        pass

    @staticmethod
    def forward(ctx, input, beta=0.25, scaling_factor=None):
        ctx.save_for_backward(input, scaling_factor)
        ctx.beta = beta
        max_val = torch.max(input)
        threshold = beta * max_val
        binary_output = torch.where(input > threshold / 1,
                                    torch.tensor(1.0, device=input.device),
                                    torch.tensor(0.0, device=input.device))

        if scaling_factor is None:
            scaling_factor = torch.mean(abs(input))
            scaling_factor = scaling_factor.detach()

        binary_output = scaling_factor * binary_output
    ...
    def backward(ctx, grad_output):
        ...
        # 根据Softmax操作调整梯度
        # 这里我们考虑Softmax操作的影响，与前向传播保持一致
        # 对于输入值大于阈值的部分，梯度保持不变；否则为0
        max_val = torch.max(input)
        threshold = beta * max_val
        ...
        return grad_input, None, grad_scaling_factor


class SoftmaxAwareBinarizationLayer(nn.Module):
    def __init__(self, beta=0.25, use_pws=False):
        super(SoftmaxAwareBinarizationLayer, self).__init__()
        self.beta = beta
        self.use_pws = use_pws
        if use_pws:
            self.scaling_factor = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, input):
        return SoftmaxAwareBinarization.apply(input, self.beta, self.scaling_factor)
...
class BinMultiheadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout, kdim=None, vdim=None,
        ...
        self.bin_bool_attnscore = SoftmaxAwareBinarizationLayer()
...
class BinTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int, dropout: float = 0.1,
        ...
        self.bin_sign3 = BinarizationSignLayer()
