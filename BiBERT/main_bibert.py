...
from BiBERT import BiBERTModel
...
def main():
    ...
    if len(args.multigpu) > 1:
        model = nn.DataParallel(BiBERTModel(input_dim=input_dim, num_classes=class_number,
                                            seq_len=train_dataset.data.max_seq_len, dropout=args.dropout),
                                device_ids=device)
    else:
        model = BiBERTModel(input_dim=input_dim, num_classes=class_number,
                            seq_len=train_dataset.data.max_seq_len, dropout=args.dropout)
