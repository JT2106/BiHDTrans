...
def main():
    ...
    if len(args.multigpu) > 1:
        model = nn.DataParallel(BiTModel(input_dim=input_dim, num_classes=class_number,
                                         seq_len=train_dataset.data.max_seq_len, dropout=args.dropout),
                                device_ids=device)
    else:
        model = BiTModel(input_dim=input_dim, num_classes=class_number,
                         seq_len=train_dataset.data.max_seq_len, dropout=args.dropout)
